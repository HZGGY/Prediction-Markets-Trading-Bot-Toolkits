...
impl Error {
    ...
    pub fn empty_response(status_code: StatusCode, method: Method, path: String) -> Self {
        EmptyResponse {
            status_code,
            method,
            path,
...
/// A successful HTTP response whose JSON value was `null` where a value was required.
#[non_exhaustive]
#[derive(Debug)]
...
impl fmt::Display for EmptyResponse {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "empty JSON response after successful HTTP {} making {} call to {}",
            self.status_code, self.method, self.path
        )
    }
}

impl StdError for EmptyResponse {}

impl From<EmptyResponse> for Error {
    fn from(err: EmptyResponse) -> Self {
